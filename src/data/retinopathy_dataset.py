"""
MediFusion AI - Diabetic Retinopathy Dataset Module
PyTorch Dataset and data loading utilities for the Diabetic Retinopathy dataset.

Dataset: Diabetic Retinopathy (Resized & Arranged) by Kaggle
Structure: ImageFolder with train/test splits, each containing severity grade subdirectories.
...
def _find_split_dirs(data_root: str):
    ...
    candidates = os.listdir(data_root) if os.path.exists(data_root) else []
...
def load_retinopathy_data(
    ...
    """
    Load Diabetic Retinopathy dataset and create train/val/test DataLoaders.

    Supports multiple directory structures:
    - data_root/train/{class_dirs}/ and data_root/test/{class_dirs}/
    - data_root/Training/{class_dirs}/ and data_root/Testing/{class_dirs}/
    - Flat data_root/{class_dirs}/ (splits from single directory)
    ...
    train_dir, test_dir = _find_split_dirs(data_root)

    # If no explicit split dirs, check for a nested level
    if train_dir is None:
        for subdir in os.listdir(data_root):
            sub_path = os.path.join(data_root, subdir)
            if os.path.isdir(sub_path):
                t, te = _find_split_dirs(sub_path)
                if t is not None:
                    train_dir, test_dir = t, te
                    break

    # Fallback: use entire data_root as a single dataset
    if train_dir is None:
        train_dir = data_root
    ...
    if test_dir and os.path.exists(test_dir):
        ...
        # Split some from training as test
    ...
    info = {
        ...
        "display_names": RETINOPATHY_CLASSES[:len(class_names)],
    ...
    logger.info(
        f"Diabetic Retinopathy dataset loaded: "
